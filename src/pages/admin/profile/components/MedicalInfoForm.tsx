...
import DoctorProfilePicForm from "./AdminProfilePicForm";
...
const MedicalInfoForm: React.FC<MedicalInfoFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      await AdminApi.updateProfile(data).then((res) => {
        if (res?.status) {
          setFeedback({
            type: "success",
          ...
          dispatch(
            updateUser({
              ...
              user: { ...userProfile?.user, ...res?.data, phoneNumber: res?.data?.contactNumber },
            })
          );
        }
      });
  ...
  return (
    <main className="bg-white  rounded-lg border border-dashboard-gray max-w-6xl mx-auto">
      <div className="flex items-center justify-between p-4 border-b border-dashboard-gray">
        <h4 className="text-2xl 2xl:text-4xl">Update Admin Information</h4>
        <IoClose

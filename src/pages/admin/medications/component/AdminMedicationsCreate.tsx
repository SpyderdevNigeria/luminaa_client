...
const handleChange = (
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (medication) {
        const response = await AdminApi.updateMedication(medication.id, {...formData, price : formData?.price});
        setMessage({
          message: response?.data?.message || "Medication updated successfully",
          type: "success",
        });
      } else {
        const response = await AdminApi.createMedication({...formData, price : formData?.price});
        setMessage({

...
export default function PatientServicesDetails() {
  ...
  useEffect(() => {
    const fetchService = async () => {
      ...
      setLoading(true);
      try {
        const res = await PatientApi.getServiceById(id);
        setService(res?.data);
      } catch (err: any) {
        console.error(err);
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-lg font-medium">₦{service?.applicablePrice || service.basePrice }</p>
          </div>
        </div>

        <div className="space-y-3">
          <div>
            <p className="text-gray-500 text-sm">Avaiable </p>
            <p
              className={`text-lg font-medium ${
                service.deletedAt ? "text-red-600" : "text-green-600"
              }`}
            >
              {service.deletedAt ? "No not at the moment" : "Yes, it's available "}
            </p>
          </div>

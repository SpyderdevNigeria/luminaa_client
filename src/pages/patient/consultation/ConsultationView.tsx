    ...
    <div className="p-4 md:p-6 bg-white shadow rounded">
        ...
        {["general", "diagnosis", "prescription"].map((tab) => (
          <button
            ...
            className={`capitalize px-4 py-2  ${
              activeTab === tab ? "border-b-2 border-primary text-primary" : "text-inactive"
            }`}
          >
            {tab}
          </button>
      ...
      {activeTab === "diagnosis" && (
        <>
          {diagnosis.length > 0 ? (
            diagnosis.map((d) => (
              <div className="mt-6  border border-dashboard-gray rounded-md p-4" key={d.id || d.diagnosisCode}>
                <h3 className="text-lg font-semibold mb-4">Diagnosis Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div><strong>Primary Diagnosis:</strong><p>{d.primaryDiagnosis}</p></div>
                  <div><strong>Symptoms:</strong><p>{d.symptoms}</p></div>
                  <div><strong>Notes:</strong><p>{d.notes}</p></div>
                  <div><strong>Severity:</strong><p>{d.severity}</p></div>
                  <div><strong>Diagnosis Code:</strong><p>{d.diagnosisCode}</p></div>
                  <div><strong>Confirmed:</strong><p>{d.isConfirmed ? "Yes" : "No"}</p></div>
                  <div className="md:col-span-2"><strong>Additional Recommendations:</strong><p>{d.additionalRecommendations}</p></div>
                </div>
              </div>
            ))
          ) : (
      ...
      {activeTab === "prescription" && (
        <>
          {prescriptions.length > 0 ? (
            prescriptions.map((p) => (
              <div className="mt-6  border border-dashboard-gray rounded-md p-4" key={p.id}>
                <h3 className="text-lg font-semibold mb-4">Prescription Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div><strong>Medication Name:</strong><p>{p.medicationName}</p></div>
                  <div><strong>Dosage:</strong><p>{p.dosage}</p></div>
                  <div><strong>Frequency:</strong><p>{p.frequency}</p></div>
                  <div><strong>Duration:</strong><p>{p.duration}</p></div>
                  <div className="md:col-span-2"><strong>Instructions:</strong><p>{p.instructions}</p></div>
                </div>
              </div>
            ))
          ) : (
